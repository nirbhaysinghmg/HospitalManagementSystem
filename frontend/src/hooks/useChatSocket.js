...
export const useChatSocket = (setChatHistory, setStreaming, customChatUrl) => {
  ...
  const chatUrl = customChatUrl || "ws://localhost:8000/ws/chat";
  ...
  const connectWebSocket = useCallback(() => {
    console.log("Attempting to connect WebSocket to:", chatUrl, "(Retry:", retryCount.current, ")");
          ...
          if (data.text) {
            setChatHistory(prev => {
              ...
              if (lastMessage && lastMessage.role === "assistant" && !lastMessage.completed) {
                ...
                return [...prev, { role: "assistant", text: data.text, completed: false }];
              }
            });
          }
          
          // Handle chunk messages (legacy format)
          if (data.chunk) {
            setChatHistory(prev => {
              const lastMessage = prev[prev.length - 1];
              if (lastMessage && lastMessage.role === "assistant" && !lastMessage.completed) {
                const updatedMessage = { ...lastMessage, text: lastMessage.text + data.chunk };
                return [...prev.slice(0, -1), updatedMessage];
              } else {
  ...
  return { ws, connectWebSocket, waitForConnection, sendMessage, connectionStatus };
};
